...
**Note**: This is an **optional lecture**. You are encouraged to take the full 5 days off this week, not just the 3 scheduled by UBC. 
...
Other tools aside from make:

- ProjectTemplate (we will cover this if we have time)
- remake for R
...
3. [ ] Add the creation of `words.txt` to the Makefile:
   ...
   ```
   Or, if this is too slow, mac users already have this file on their computer and can get it by copying the file over, like so:
   ```
...
5. [ ] Add the tsv output to the makefile:
   ...
   ```
      ...
      - What happens if a dependency is not present? Try deleting `words.txt`.
6. [ ] Create rules for `all` and `clean` (why don't we need `words.txt` listed for `all`?)
   ```
   all: histogram.tsv
