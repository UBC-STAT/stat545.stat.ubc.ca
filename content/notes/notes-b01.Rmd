...
## Using functions with dplyr scoped variants

Sometimes we need to specify a function on-the-fly. We explore making functions in STAT 545B, but we encourage you to try this challenge. With `across()`, specify your homemade function starting with `~` and using `.x` or `.` as the function argument. So, `~ sum(is.na(.x))` calculates how many NA values there are in each column.
...
- [STAT 545 Episode 1-B: R Functions for Data Analysis](https://youtu.be/3nDgR7l5Tps)
