...
We will spend 1-2 days on this topic.
...
The tidy data illustrations in the rest of this document are from the [Openscapes](https://www.openscapes.org/) blog "[Tidy Data for reproducibility, efficiency, and collaboration](https://www.openscapes.org/blog/2020/10/12/tidy-data/)" by [Julia Lowndes](https://jules32.github.io/) and [Allison Horst](https://www.allisonhorst.com/). 
...
2. As a group, mentally (or with pen and paper) sketch out the format of the tidy tibble `drinks_tbl2`. 

## How to tidy your data 

### Step 1: What are the underlying variables and observations? 
Step 1 is to figure out what the underlying variables and observations are. This might seem straightforward, but it can be surprisingly hard because it **depends on the (statistical) question being asked**. 

#### Example: Great British Bakeoff

We will demonstrate using data from "The Great British Bake Off" compiled by [Allison Hill](https://www.apreshill.com/) in the R package `bakeoff`. (Note that this package is not hosted on CRAN, so if you would like to install it, you will need to run `install.packages("remotes")` then type `remotes::install_github("apreshill/bakeoff")` into R.) The code that follows was lightly adapted from Allison's [Plot Twist talk](https://www.apreshill.com/talk/2019-rladies-sydney/). 
...
Notice that our observations and variables have changed! Our observations are now *series*, and our relevant *variables* are now the number of viewers in millions within a 7-day window for the first and last episode of each series. Given these variables, we could calculate the variable we display in the bar plot (the percentage increase in viewers from the premiere to finale) with the logic `(last-first)/first`. In other words, the tidy data representation is this wider format: 

```{r, echo=FALSE}
head(ratings_tbl2) 
```

The moral of the story: what is "tidy" can differ for **the exact same data set**. This is because what is an observation and what is a variable depends on the statistical question being asked! Once again, we return to the words of the great philosopher Aretha Franklin ...
...
> "Of course, it is very easy to disregard people you have never met, and who are certainly not your friends or family members. After all, in the eyes of an outsider, who is in no danger whatsoever, the people caught up in the situation are nothing more than simply statistics." - Andrew James Pritchard
...
These are important conversations for us as a data science community to have. 
