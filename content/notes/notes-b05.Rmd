...
3. It's almost always better to write an `.Rmd` file over an `.R` file, because you're able to interlace markdown. 
...
We'll now run through the [makefile activity](https://stat545.com/automating-pipeline.html) included in the stat545.com book. Here is a digested version of the activity.
...
4. [ ] Create rules for `all` and `clean`:
   ```
   all: words.txt
...
5. [ ] Make a new R script that makes a table of word lengths -- [code here](https://raw.githubusercontent.com/STAT545-UBC/STAT545-UBC.github.io/master/automation10_holding-area/activity/histogram.r).
6. [ ] Add the tsv output to the makefile:
   ```
...
7. [ ] Update rules for `all` and `clean`.
8. [ ] Plot a histogram of word lengths, direct in the Makefile.
   ```
   histogram.png: histogram.tsv
     Rscript -e 'library(ggplot2); qplot(Length, Freq, data=read.delim("$<")); ggsave("$@")'
   ```
9. [ ] Update: add a `rm Rplots.pdf` line to the histogram creation rule; update `all` and `clean`.
10. [ ] Make a `report.Rmd` file -- [code here](https://raw.githubusercontent.com/STAT545-UBC/STAT545-UBC.github.io/master/automation10_holding-area/activity/report.rmd).
11. [ ] Add a rule to make the report output:
    ```
