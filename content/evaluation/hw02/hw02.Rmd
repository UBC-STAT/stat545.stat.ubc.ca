...
Use the pipe operator `%>%` to select "country" and "gdpPercap" from your filtered dataset in 3.1.
...
Filter gapminder to all entries that have just experienced a drop in life expectancy. Be sure to include a new variable that's the increase in life expectancy. Hint: you might find the `lag()` or `diff()` functions useful. 
...
Make two plots that have some value to them. That is, a plot that someone might actually consider making for an analysis.

* A scatterplot of two quantitative variables (not the same ones we made in class -- feel free to use a data set from the `datasets` R package).
