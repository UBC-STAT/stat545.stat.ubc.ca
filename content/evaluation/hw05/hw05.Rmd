...
<!-- I think the first sentence of the above point is confusing. Suggest change to: "Explore the effects of re-leveling a factor in a tibble by: 1) comparing the results of `arrange` on the original and re-leveled factor. 2) Plotting a figure of before/after re-leveling the factor (make sure to assign the factor to an *aesthetic* of your choosing). --> 
...
<!-- Consider changing "a principled summary" to "summarized information" or "summary statistic". Maybe also exclude the mean, since that's a pretty basic statistic too?--> 
...
<!-- Are we not encouraging use of forcats package for both of these options? It seems that this hint could go to the beginning of this exercise. Maybe it shouldn't be a hint even, and instead explicitly state to not use `as.factor` or `factor` functions since the point here is to learn forcats. -->
...
<!-- What about requiring use of the `here` package here?-->
...
<!-- Is this second task supposed to use the reloaded data of the first task, or is it standalone? I'm not sure what is mean't by "make your data non-alphabetical"-->
