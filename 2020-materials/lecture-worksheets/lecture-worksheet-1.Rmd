...
```{r test-1-1}
# Test that the digested version matches.
```
...
```{r answer-1-2}
# Check that digest(as.numeric(answer1.2)) matches the digested answer.
```
...
```{r test 1-3}
# Test that the digested form matches.
```
...
```{r test-1-4}
# Test that digested version matches.
```
...
```{r test-1-7}
# Test against digested version of both
```
